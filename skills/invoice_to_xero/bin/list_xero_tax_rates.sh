...
# list_xero_tax_rates.sh — List Xero tax rates
...
URL="${M365_XERO_BASE_URL}/v1/xero/tax-rates?connection_id=${CONNECTION_ID}"
[[ -n "$STATUS" ]] && URL="${URL}&status=${STATUS}"

curl -sf \
  -H "Authorization: Bearer ${M365_XERO_API_KEY}" \
  "${URL}" \
  || { echo "curl failed (exit $?): request to /v1/xero/tax-rates failed" >&2; exit 1; }
